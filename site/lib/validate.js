 ...
 */
import { BODY_AXES, BRAIN_AXES, BRAIN_TIERS, GENOME_VERSION, HEART_TIERS, PART_AXES, SLOT_NAMES, } from "./genome.js";
...
export function validateGenome(g) {
    ...
    for (const s of SLOT_NAMES) {
        if (!present.includes(s))
            errors.push(`missing slot: ${s}`);
    }
    for (const s of present) {
        if (!SLOT_NAMES.includes(s)) {
            errors.push(`unknown slot: ${s}`);
            continue;
        }
        const allele = g.slots[s];
        if (!allele)
            continue;
