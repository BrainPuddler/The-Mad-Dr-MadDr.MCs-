 ...
 */
import { HEART_OUTPUT, SLOT_NAMES, bodyAxis, brainSize, heartVigor, partAxis, } from "./genome.js";
import { BODY_PLANS, express, isVestigial, originOf } from "./catalog.js";
export const ENERGY_TYPES = ["blood", "fuel", "ichor"];
export const ORIGIN_ENERGY = {
    organic: "blood",
    tech: "fuel",
    biotech: "ichor",
};
 ...
 * arm gulps). */
export function partUpkeep(allele) {
...
export function upkeep(g) {
    const out = { blood: 0, fuel: 0, ichor: 0 };
...
export function heartCapacity(heart) {
...
export function circulatoryLoad(g) {
    const u = upkeep(g);
...
export function viability(g) {
    const capacity = heartCapacity(g.heart);
    const load = circulatoryLoad(g);
    const margin = capacity - load;
    const state = load <= capacity ? "viable" : load <= capacity * SHOCK_FACTOR ? "strained" : "nonviable";
...
export function reanimationSurge(g) {
    const u = upkeep(g);
