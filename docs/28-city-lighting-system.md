...
| 39 | Screenshot + question, not a direction: "are these rim lights without the full window light on purpose or a bug" — a warm sill glow clearly visible beside several windows whose own panes rendered fully dark | Real, pre-existing bug, not a design choice — found by reading `GlowPointRegistry`/`DynamicLightBudget` directly. A window's actual on/off state has been entirely GPU-side since docs/33 shipped, but `BuildingWindowGrid.Build()` registers every glow-CAPABLE window for the shared Tier-2 real-light budget unconditionally, with no live check of whether that specific window is CURRENTLY rendering lit — `RegisterPosition`'s own prior doc comment said as much ("decided once at registration"), true for can-glow, silently wrong for currently-glowing. `DynamicLightBudget`'s selection loop can therefore promote a real light at any registered window position regardless of that window's instantaneous state, so `FacadeKit.WindowBay`'s sill prop (originally written to complement a lit window, "catches light and reads at distance") can catch a real light next to a pane that's OFF. Pre-existing since docs/33, but row 37's hard-switch change made it far more visually obvious than the old smoothstep fade did | `GlowPointRegistry.RegisterPosition` gained the same `isEligible` predicate parameter `Register` (the Transform-based overload, built for traffic headlights) already had. `BuildingWindowGrid` now retains a small per-window `WindowSchedule` array and passes a live closure, `IsWindowOnNow`, that mirrors `WindowGrid.shader`'s `MadDrWindowMultiplier` gate exactly (override texture -- skipped via a `_hasOverride` flag unless this building has ever had `SetWindowOn`/`Off` called, since `Texture2D.GetPixel` isn't free at this call frequency -- then canGlow, then the arrival/bedtime/activity schedule against live `DayNightState`) | Code-level addition to `DynamicLightBudget.cs` + `BuildingWindowGrid.cs` — same standing "no Editor here" verification ceiling; not yet seen in a real render |
