...
**Implementation note (found building this): `Vitality`/`Armor`/`sizeClass` never shipped.** The genome v2 schema actually adopted (Q10, `packages/genome-core/src/genome.ts`) has no `statGenes` block at all — no `Vitality`, `Power`, `Armor`, `Reach`, `Speed`, `Ferocity`, or `Cunning` gene, and no `sizeClass`. Those are docs 04/06's aspirational stat-block vocabulary from before the v2 schema replaced it; the JSON sketch earlier in this doc (`"statGenes": {...}`) is now stale and needs a follow-up correction, tracked informally here rather than as a new Q since it's a documentation-accuracy fix, not an open design question. The shipped formula, `bonesCost()` in `packages/genome-core/src/cost.ts`, computes the same "bigger/tougher costs more" intent from the fields that DO exist — body `bulk`, heart tier, and summed limb `length × girth` per slot (mirroring how [17](17-factions.md)'s Motive-class formula already prices limbs) — not literally `4×sizeClass + 0.1×Vitality + 2×Armor`. The worked examples above are illustrative of the *shape* (small vs. tank-sized spread), not exact numbers a real genome will reproduce.
...
**Formula (v0.1, revised to match the shipped implementation)**: every slot and the heart are stripped into the surgical tray at **100% recovery** — Cannibalize reuses the exact same `harvestPart`/`harvestHeart` functions surgery already runs ([below](#grafting-as-surgery-cut-parts-off-sew-them-on)), so nothing about a limb or organ is lost, only the frame holding them together. On top of that, Cannibalize pays a **50% Bones bonus** on the design's own Bones-cost bill (above, "The bill") — the frame itself, which surgery alone doesn't recover. (This revises the original design, which proposed 50% recovery across the board including Parts and a Brain-tier roll; building it surfaced that reusing the lossless harvest system for Parts is strictly better and cost nothing extra to implement, and that the shipped genome schema has no discrete Brain-tier-item field to roll for at all — see the note on `bonesCost` below.)
