...
| [26-special-attacks-system.md](26-special-attacks-system.md) | **Approved architecture**: modular, cooldown-gated special attacks for enemy AI (ScriptableObject definitions, per-unit instances, Mass-based target classification), worked example Arachnid Web Attack (capture/pull human-scale targets, slow heavy ones), phased 1–8 | Phases 1–5 implemented (state-machine wiring + Web Attack targeting/AoE/classification + generic heavy-target slow effect on UnitCombat + possessed-unit friendly-fire exception hook; no pull/consume yet); Phase 6+ not started |
