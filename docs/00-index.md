...
| [27-sim-view-migration-plan.md](27-sim-view-migration-plan.md) | **Approved architecture**: the Unity-side half of docs/23 §13 amendment A — how `MonsterAgent` cuts over to rendering interpolated `match-core` sim state instead of deciding movement itself, one order kind at a time (Move first), never a single blind rewrite of the whole ~950-line file | **Phase A implemented** (`SimBridge` fixed-tick pump + command queue, `SimUnitView` interpolated render + measured-delta velocity, `MonsterAgent.EnableSimDriven`/`OrderMoveViaSim`/`TickMoveViaSim` — `WaypointCommander` needed zero changes, a smaller seam than planned); opt-in only, no existing scene affected; verified by a 15-check standalone numeric harness + flightcheck; `RuntimeCityBuilder` gained a `simDrivenDemo` Inspector toggle (default off) that opts the first spawned monster in, reusing the existing left-click/right-click workflow — **the actual Editor check, ready but not yet run**, the explicit gate before Phase B (queued/grouped moves) |
