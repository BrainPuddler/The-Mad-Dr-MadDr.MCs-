...
| [27-sim-view-migration-plan.md](27-sim-view-migration-plan.md) | **Approved architecture**: the Unity-side half of docs/23 §13 amendment A — how `MonsterAgent` cuts over to rendering interpolated `match-core` sim state instead of deciding movement itself, one order kind at a time (Move first), never a single blind rewrite of the whole ~950-line file | **Phases A–B implemented.** Phase A: `SimBridge` fixed-tick pump + command queue, `SimUnitView` interpolated render + measured-delta velocity, `MonsterAgent.EnableSimDriven`/`OrderMoveViaSim`/`TickMoveViaSim`; opt-in only; **Editor-confirmed (2026-07)** via the `simDrivenDemo` toggle on `RuntimeCityBuilder`. Phase B: single-unit waypoint queueing only (`CommandKind.MoveQueue`, `SimUnit._waypointQueue`, `SimBridge.QueueWaypointCommand`) — group moves (multi-unit formations/settle/`GroupFacing`) explicitly deferred, `WaypointCommander` needed zero changes either phase; verified by 23 match-core tests + a 16-check standalone numeric harness + flightcheck, but **the Editor check for queued (shift-click) moves specifically has not been run yet** — only Phase A's plain single-destination move has a live confirmation so far |
