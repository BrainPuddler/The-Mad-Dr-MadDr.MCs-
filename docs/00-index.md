...
| [26-special-attacks-system.md](26-special-attacks-system.md) | **Approved architecture**: modular, cooldown-gated special attacks for enemy AI (ScriptableObject definitions, per-unit instances, Mass-based target classification), worked example Arachnid Web Attack (capture/pull human-scale targets, slow heavy ones), phased 1–8 | Phases 1–7 implemented (state-machine wiring + Web Attack targeting/AoE/classification + generic heavy-target slow effect + possessed-unit friendly-fire exception hook + CaptureState pull-toward-captor + consume-on-arrival for captured citizens, reusing OnCitizenEaten; non-Citizen consume path designed, not built); Phase 8 (AI decision heuristic) not started |
