...
| [26-special-attacks-system.md](26-special-attacks-system.md) | **Approved architecture**: modular, cooldown-gated special attacks for enemy AI (ScriptableObject definitions, per-unit instances, Mass-based target classification), worked example Arachnid Web Attack (capture/pull human-scale targets, slow heavy ones), phased 1–8 | **All 8 phases implemented** (state-machine wiring, Web Attack targeting/AoE/classification, generic heavy-target slow + possessed-unit friendly-fire hook, CaptureState pull-toward-captor + consume-on-arrival for citizens, and an AcquireTarget-integrated AI decision heuristic scoring candidates by weighted AoE catch count); a web-captured citizen now credits the harvest tank the same as a direct chase-and-eat kill (2026-07 follow-up); remaining follow-ups (not part of this plan): no creature is actually equipped with a SpecialAttackDefinition asset yet (an Editor drag-and-drop step), non-Citizen consume path designed not built |
