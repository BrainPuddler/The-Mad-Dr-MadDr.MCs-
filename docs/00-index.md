...
| [26-special-attacks-system.md](26-special-attacks-system.md) | **Approved architecture**: modular, cooldown-gated special attacks for enemy AI (ScriptableObject definitions, per-unit instances, Mass-based target classification), worked example Arachnid Web Attack (capture/pull human-scale targets, slow heavy ones), phased 1–8 + Phase 9 (secondary attacks for all races) | **All 8 phases + Phase 9 implemented**: every Tank and every monster is now actually equipped (Tanks get a damage-only Flamethrower; alien-tech-handed monsters get a Psionic Tractor Beam, the same PullAndConsume mechanic as Web Attack at shorter range; every other monster gets a new Stun-effect Ground Stomp), closing the "nothing is equipped" gap for good via runtime `ScriptableObject.CreateInstance`, no Editor step needed; a web-captured citizen credits the harvest tank the same as a direct chase-and-eat kill (2026-07 follow-up); remaining follow-ups (not part of this plan): non-Citizen consume path designed not built, and "into the Lab" (site/) was read narrowly and not built — the Lab has no combat-stat display of any kind today, would need a first-of-its-kind genome-core TS twin for `Combat.WeaponFor`'s logic |
