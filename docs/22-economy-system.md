...
- **The six families render on the battlefield** — `creature-mesh` gained real geometry for all of them (sucker-mouth-with-tooth-rings, saw blade on a boom, pulsing siphon tubes, a fluid-filled sac, a riveted tank with a sight gauge, a glowing vesicle cluster), so a bred harvester looks like one in the field, not a default stub. **Storage vessels mount on the creature's BACK** (a dorsal tank, derived generically from each plan's own torso depth), never the head where sense organs go, and their **contents read RED for blood / WHITE for bone** by the harvest tool — a glance tells you what a hauler is carrying.
