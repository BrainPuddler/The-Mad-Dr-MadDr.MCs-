...
- **Herding/wander** (2026-08 follow-up, creator direction: "Add a herding behaviour to the workers. in groups of 3 to 10 and a wander toggle enabled if there is nothing to do radius around our buildings of 2 km"): when NEITHER auto-scavenge nor auto-build finds anything (`Worker.TryFindRealWork` returns false), a Worker no longer stands frozen — it either joins a nearby herd already wandering (`RuntimeCityBuilder.TryFindJoinableHerd`, a decentralized single-pass proximity check, not a real membership registry — "3 to 10" is therefore a soft target the join-preference nudges toward, not a hard-guaranteed range) or seeds a fresh casual-pace wander step of its own, leashed to within 2 km (`HexCoord.HexMeters` confirms world units are meters) of ANY player-0 building via `RuntimeCityBuilder.IsWithinRangeOfOwnBuildings`/`NearestOwnBuildingPosition`. Re-checks for real work every repick (~4s) or on arrival, so a wandering herd drops out and goes to work the instant something's nearby — "wander toggle enabled IF there is nothing to do" is bidirectional, not a one-way switch. `Worker.PickWanderTarget` retries several candidate directions before ever falling back, and its fallback always still moves — never stands still — after an early version froze in a dense city block (docs/12's same-day follow-up entry).
