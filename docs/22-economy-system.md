...
- **Auto-build**: idle Workers seek the local human's own nearest unstaffed `UnderConstruction` `SimBuilding` and, once in reach, staff it — construction genuinely doesn't progress without one there (new `SimBuilding.IsStaffed`, defaults `true` for safety so AI opponents and every pre-existing test are unaffected; only Unity ever sets it `false`, and only for player 0's own buildings, the instant a fresh build is noticed).
- **Auto-scavenge**: falls back to nearest destroyed-civilian-building debris (the same `NearestScavengeableBuildingTo`/`Building` system §11's harvester Monsters already use) only when nothing needs staffing — never citizens ("monsters would still collect resources").
