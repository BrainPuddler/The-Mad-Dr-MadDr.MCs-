...
1. Bridge dressing: trusses, piers into the riverbed, road continuity.
2. Knock-downable street furniture (poles/cars react to monster steps).
3. Damage smoke/fire plumes on Damaged buildings; dust puff on collapse.
4. Neon night mode: emissive signage pass + dusk lighting preset.
5. Rubble piles with silhouette (not just crushed cubes).
6. Railyard/industrial district dressing keyed to `rail_depot`.
7. Billboard art: period-poster quads ("ATOMIC COLA", movie one-sheets).
8. Miniature-set border: table edge / painted-backdrop skybox ring.
9. Citizen vehicles that drive road hexes and flee (docs/19 traffic).
10. Per-district palettes (downtown vs suburbs) from road-graph radius.
