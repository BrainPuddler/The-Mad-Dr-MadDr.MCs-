 ...
 * for). */
const GUN_LIKELY_PLANS: ReadonlySet<string> = new Set(["winged", "avian"]);
...
export function handFamilyWeightForPlan(family: string, plan: string): number {
  const base = weightOf(family);
  if (family === GUN_FAMILY && GUN_LIKELY_PLANS.has(plan)) return base * 6;
  return base;
}
