...
/// docs/37/docs/12's win/loss-states pass (Elimination/Dominion/TimeCap)
/// gave match-core a real, queryable verdict-in-progress for each
/// condition, but nothing showed it to the player before the match
/// actually ended (<see cref="MatchEndHud"/> only appears once it's
/// already over). This is that live readout -- three rows, one per
/// docs/02 victory condition, each a percentage colored green (&gt;=50%,
/// favoring the local player) or red (&lt;50%), docked directly ABOVE
/// <see cref="Minimap"/>'s own live screen rect (same "read a
/// neighbour's rect, dock against it" idiom SelectionHud/RecallHud
/// already use for that corner).
///
...
public class WinProgressHud : MonoBehaviour
{
    ...
    public void Init(SimBridge simBridge, Minimap minimap, int localPlayerIndex)
    {
        _bridge = simBridge;
        _minimap = minimap;
    ...
    private const float DockGapPixels = 6f;
    ...
    private void OnGUI()
    {
        if (_bridge == null || _minimap == null || !_bridge.HasMatch || _bridge.IsMatchOver) return;
        var prevMatrix = UiScale.Begin();

        var mapRect = _minimap.ScreenRect;
        var panelHeight = RowHeight * 3f + Pad * 2f;
        var panelRect = new Rect(mapRect.x, mapRect.y - panelHeight - DockGapPixels, mapRect.width, panelHeight);

        GUI.color = PanelColor;
        GUI.DrawTexture(panelRect, Texture2D.whiteTexture);
        GUI.color = Color.white;

        var y = panelRect.y + Pad;
        DrawRow(panelRect, ref y, "Army", EliminationPercent());
        DrawRow(panelRect, ref y, "Dominion", DominionPercent());
        DrawRow(panelRect, ref y, "Territory", TerritoryPercent());

        UiScale.End(prevMatrix);
    }

    private void DrawRow(Rect panelRect, ref float y, string label, float percent)
    {
        var labelRect = new Rect(panelRect.x + Pad, y, LabelWidth, RowHeight);
        GUI.Label(labelRect, label);

        var valueRect = new Rect(labelRect.xMax, y, panelRect.width - LabelWidth - Pad * 2f, RowHeight);
